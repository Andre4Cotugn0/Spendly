...
class QuickExpenseWidgetService {
  static final QuickExpenseWidgetService instance =
  ...
  Future<void> _initializeAndroid() async {
    try {
      ...
      final categoriesJson = categories
          .map(
            (c) =>
                '''{{"id":"${c.id}","name":"${c.name}","color":"${c.color.toARGB32()}","icon":"${c.iconName}"}}''',
          )
          .join(',');

      await HomeWidget.saveWidgetData<String>('categoriesJson', categoriesJson);
      await updateWithMonthTotal();
  ...
  Future<void> updateWithMonthTotal() async {
    ...
    try {
      ...
      final total = await DatabaseHelper.instance.getTotalByMonth(
        now.year,
        now.month,
      );
